sap.ui.define([
	...
	return BaseObject.extend("Coxinha_Hana.Coxinha.v02.model.GroupSortState", {
		 ...
		 * @alias Coxinha_Hana.Coxinha.v02.model.GroupSortState
		 */
		...
		/**
		 * Groups by valor, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "valor") {
				...
				this._oViewModel.setProperty("/sortBy", "valor");

				aSorters.push(
					new Sorter("valor", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "id_pro");
			}
